...
RSpec.describe Prog::Postgres::PostgresServerNexus do
  ...
  describe ".assemble" do
    ...
    it "sizes a replacement wal volume from the representative's grown volume" do
      aws_resource = create_postgres_resource(project: user_project, location_id: aws_location.id)
      aws_resource.update(storage_type: "network_cache", network_volume_type: "gp3", wal_drive_type: "io2")
      Firewall.create(name: "#{aws_resource.ubid}-internal-firewall", location: aws_location, project: service_project)
      postgres_timeline = create_postgres_timeline(location_id: aws_location.id)

      first = described_class.assemble(resource_id: aws_resource.id, timeline_id: postgres_timeline.id, timeline_access: "push", is_representative: true)
      # Stand in for resize_wal_volume having doubled the volume twice.
      first.subject.wal_volume.update(size_gib: 128)

      second = described_class.assemble(resource_id: aws_resource.id, timeline_id: postgres_timeline.id, timeline_access: "fetch")
      wal_volume = second.subject.vm.strand.stack.first["storage_volumes"].last
      expect(wal_volume).to include("size_gib" => 128, "network_volume_type" => "io2")
    end
  ...
  describe "#resize_wal_volume" do
    ...
    before do
      # Network WAL only exists on a provider location; metal has no volume row.
      server.vm.location.update(provider: "aws")
      postgres_resource.update(storage_type: "network_cache", network_volume_type: "gp3", wal_drive_type: "gp3")
      wal_volume
    end

    it "clamps the growth step to the provider cap" do
      # gp3 caps at 16 TiB; the trigger only checks the current size is under it.
      wal_volume.update(size_gib: 12288)
      expect(server).to receive(:grow_wal_volume).with(16384).and_return(true)
      expect { nx.resize_wal_volume }.to hop("wait_wal_volume_resized")
      expect(wal_volume.reload.size_gib).to eq(16384)
    end
