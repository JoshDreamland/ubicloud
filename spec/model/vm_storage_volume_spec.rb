...
RSpec.describe VmStorageVolume do
  ...
  it "can render a device_path for aws" do
    ...
    nv = NetworkVolume.create(location_id: vm.location.id, size_gib: 64, provider_id: "vol-0wal456")
    expect(described_class.new(disk_index: 2, vm:, network_volume: nv).device_path).to eq("/dev/disk/by-id/nvme-Amazon_Elastic_Block_Store_vol0wal456")
  end
