...
RSpec.describe Clover, "postgres/capabilities" do
  ...
  describe "authenticated" do
    ...
    it "includes size, storage_type, volume chain and storage_size levels" do
      ...
      expect(family_tree["size"][size_key].dig("storage_type", "instance_storage", "network_volume_type", "none")).to include("storage_size")
    end
    ...
    it "includes ha_type as leaf" do
      ...
      family_tree = tree.dig("flavor", "standard", "location", "hetzner-fsn1", "family", "standard")
      size_key = family_tree["size"].keys.first
      storage_size_tree = family_tree["size"][size_key].dig("storage_type", "instance_storage", "network_volume_type", "none", "storage_size")
