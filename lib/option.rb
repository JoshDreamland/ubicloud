...
module Option
  ...
  # lssd: false is only valid for families with a non-lssd base type
  # (c4a/c4/c4d). z3 is local-SSD-only, but is postgres-only so it always
  # carries data volumes and never resolves to lssd: false.
  ...
  # GCE live migrates during host maintenance only up to this much attached
  # local SSD; above it the instance must terminate and restart. z3 rejects an
  # insert whose policy disagrees, and no shape falls between this and double
  # it, so one threshold covers every family.
