...
class Prog::Vm::Nexus < Prog::Base
        ...
        DB.ignore_duplicate_queries do
          storage_volumes.each_with_index do |volume, disk_index|
            VmStorageVolume.create(
              ...
              use_bdev_ubi: false,
              disk_index:,
              **network_volume_attrs(location, volume),
            )
          end
        end
        "Vm::Aws::Nexus"
  ...
  def self.network_volume_attrs(location, volume)
    ...
    network_volume = NetworkVolume.create(location_id: location.id, size_gib: volume[:size_gib])
    config = {
      volume_type:,
      provisioned_iops: volume[:provisioned_iops],
      provisioned_throughput_mibps: volume[:provisioned_throughput_mibps],
    }
    (location.aws? ? AwsVolume : GcpVolume).create_with_id(network_volume, **config)
