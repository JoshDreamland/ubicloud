...
SQL
  ...
  on_failure :start, "Postgres server failed to begin provisioning."
  on_failure :bootstrap_rhizome, "Postgres server VM is not responding to SSH bootstrapping."
  on_failure :wait_bootstrap_rhizome, "Postgres server VM bootstrap did not complete in time."
  on_failure :mount_data_disk, "Postgres server data disk could not be mounted."
  on_failure :run_init_script, "Postgres server initialization script did not complete in time."
  on_failure :configure_walg_credentials, "Postgres server WAL-G credentials could not be configured."
  on_failure :initialize_empty_database, "Postgres empty database initialization did not complete in time."
  on_failure :initialize_database_from_backup, "Postgres database restore from backup did not complete in time."
  on_failure :refresh_certificates, "Postgres server TLS certificate refresh did not complete in time."
  on_failure :configure_metrics, "Postgres server metrics agent could not be configured."
  on_failure :setup_cloudwatch, "Postgres server CloudWatch agent setup did not complete in time."
  on_failure :setup_hugepages, "Postgres server hugepages configuration did not complete in time."
  on_failure :configure, "Postgres configuration (postgresql.conf / pg_hba.conf) failed to apply."
  on_failure :update_superuser_password, "Postgres superuser password update did not complete in time."
  on_failure :run_post_installation_script, "Postgres post-installation script did not complete in time."
  on_failure :wait_catch_up, "Standby is not replicating WAL fast enough to catch up to the primary."
  on_failure :wait_synchronization, "Standby did not achieve synchronous replication in time."
  on_failure :wait_recovery_completion, "Postgres standby did not finish crash recovery in time."
  on_failure :wait, "Postgres server is stuck in steady state; health checks or failover may be failing."
  on_failure :unavailable, "Postgres server did not recover from unavailable state in time."
  on_failure :fence, "Postgres primary fencing did not complete in time during failover."
  on_failure :wait_in_fence, "Postgres primary is stuck in the fenced state; failover may be stalled."
  on_failure :prepare_for_unplanned_take_over, "Postgres unplanned failover preparation did not complete in time."
  on_failure :wait_representative_lockout, "Postgres primary lockout did not begin in time during failover."
  on_failure :lockout, "Postgres primary lockout could not be applied."
  on_failure :wait_lockout_attempt, "Postgres primary lockout attempt did not succeed in time."
  on_failure :wait_locked_out, "Postgres primary is locked out; waiting for standby to promote."
  on_failure :prepare_for_planned_take_over, "Postgres planned failover preparation did not complete in time."
  on_failure :wait_fencing_of_old_primary, "Old primary is not being fenced in time during planned failover."
  on_failure :promote_read_replica, "Postgres read replica promotion did not complete in time."
  on_failure :taking_over, "Postgres server takeover (promotion) did not complete in time."
  on_failure :destroy, "Postgres server could not be destroyed."
  on_failure :wait_children_destroy, "Postgres server child strands are not completing destruction in time."
  on_failure :destroy_vm_and_pg, "Postgres server VM teardown did not complete in time."
end
